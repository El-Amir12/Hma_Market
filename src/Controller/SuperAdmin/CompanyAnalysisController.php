...
class CompanyAnalysisController extends AbstractController
{
    ...
    public function uploadFinalReport(
        ...
        try {
            $exportService->uploadFinalReport(
                $analysisRequest,
            ...
            $this->logger->error('Erreur upload rapport final', [
                'analysis_id' => $analysisRequest->getId(),
                'error' => $e->getMessage()
            ]);
        }
    ...
    public function replaceFinalReport(
        ...
        try {
            $exportService->updateFinalReport(
                $analysisRequest,
                $uploadedFile->getPathname(),
                $uploadedFile->getClientOriginalName(),
                $this->getUser()
            ...
            $this->logger->error('Erreur remplacement rapport final', [
                'analysis_id' => $analysisRequest->getId(),
                'error' => $e->getMessage()
            ]);
        }
    ...
    public function downloadFinalReport(
        ...
        return $this->file($result['path'], $result['filename']);
    }
